...
### 3. Classification is the binding constraint, and the demo corpus hid it

The heuristic scores **100% on `demo/queries.jsonl` and 31.7% on held-out data**
(`demo/eval_classification.jsonl`, 60 naturalistic queries, 16 task types). The
gap is not subtle, and it has one cause: the demo corpus was written first and
the regexes were tuned until they matched it. That is curve-fitting, not
measurement. Run it yourself: `python demo/eval.py`.

The architectural consequence is that **the regex layer is not the classifier —
it is a spend gate.** The real classifier is the LLM call it decides whether to
pay for. What matters is therefore not accuracy but *calibration*: does high
confidence actually mean correct?

| | Queries | Accuracy |
|---|---|---|
| confidence >= 0.55 — commit, no LLM call | 23% | **92.9%** |
| confidence < 0.55 — pay for the classifier | 77% | 13.0% |

A 79-point separation means the gate reliably knows what it does not know.
Getting there required fixing the confidence formula, which weighted the
winner's *margin* over rivals without regard to absolute evidence — so a single
weak pattern match with nothing competing scored 0.82. "Nothing else matched"
was being read as "I am sure", and that band held every silent misroute (stated
0.83, actual 0.60). Confidence is now gated by absolute evidence, and separation
can only amplify evidence that already exists. Silent misroutes: 7 -> 1.

The cost of honesty is a higher classifier fire rate (40% -> 77%). It is
affordable: at $0.00072 per call, classifier overhead is **1.6% of the average
per-query saving** ($0.000552 against $0.0346). Paying Haiku to classify is
roughly two orders of magnitude cheaper than one misrouted Opus call.

> **Disclosure:** the confidence formula was rebalanced *after* seeing these
> held-out results, so `eval_classification.jsonl` is now contaminated too. The
> 92.9% gate figure is tuned-on-set and should be re-measured against fresh
> queries before anyone relies on it. The 31.7% raw accuracy was measured before
> any change and stands.
